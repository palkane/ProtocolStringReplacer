    ...
    private PacketContainer clonePacket(PacketContainer packet) {
        try {
            ...
            if (exceptionTimes < ProtocolStringReplacer.getInstance().getConfigManager().protocolLibSideStackPrintCount) {
                ProtocolStringReplacer.warn("Exception from the embedded protocol layer: " + e);
                ProtocolStringReplacer.warn("Please update ProtocolStringReplacer to the latest version.");
                exceptionTimes++;
            }
            return null;
        }
